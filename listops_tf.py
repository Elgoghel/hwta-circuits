"""
Tree-input Transformer baseline for ListOps.

Same tree-structured input as listops.py. Same architecture skeleton as
cruxmini_tf.py, adapted for 4-op ListOps grammar with MAX held out.

If the transformer also hits 100% on compound-MAX generalization, the V4
circuit's win is trivial. If it collapses the way it did on SCAN and
CruxMini, the content-structure separation story extends cleanly to a
third task class (comparison + aggregation ops).

Usage:
    python listops_tf.py              # full
    python listops_tf.py --smoke      # quick
"""
...
from listops import (
    ...
    make_listops_batch, accuracy,
)


@dataclass
class ListOpsTFConfig:
    d_model: int = 128
    n_heads: int = 4
    n_layers: int = 4
    ff_mult: int = 4
...
class TreeTransformer(nn.Module):
    """Vanilla transformer over listops trees.
    ...
    Matched architecture to cruxmini_tf's TreeTransformer. Only difference
    is n_ops = 4 (vs 3 in cruxmini), which adds ~1% more params.
    """
    def __init__(self, cfg: ListOpsTFConfig):
        super().__init__()
        self.cfg = cfg
        d = cfg.d_model
        ...
        self.op_embed = nn.Embedding(cfg.n_ops, d)          # 4 ops: min/max/sum/prod
...
def main():
    ...
    print("=" * 66)
    print("ListOps -- Tree Transformer baseline")
    print("=" * 66)
    ...
    print("\n" + "=" * 66)
    print("FINAL RESULTS -- Tree Transformer on ListOps")
    print("=" * 66)
    print(f"  Train acc (leaf-level MAX):    {final['acc_train']:.3f}")
    print(f"  Test_max acc (compound MAX):   {final['acc_test_max']:.3f}")
    print(f"  Training time:                 {t_train:.1f}s")
    print("=" * 66)
    print("\nReference: ListOps V4 circuit (4000 params): 1.000 / 1.000 (expected)")
    print()

    out_tag = f'listops_tf_d{cfg.d_model}_l{cfg.n_layers}_s{args.seed}'
